...
module.exports = function (describe, it, assert, assertEqual) {
    ...
    describe('Circuit grafo → buscador → huella — deep-links', function () {
        ...
        it('siteSurface casos strip — progresivo 4+3 (design thinking)', function () {
            var js = fs.readFileSync(path.join(web, 'lib', 'siteSurface.js'), 'utf8');
            assert(js.indexOf('PRIORITY_CASO_IDS') !== -1, 'prioriza 4 casos tesis');
            assert(js.indexOf('data-casos-toggle') !== -1, 'toggle ver más casos');
            assert(js.indexOf('sortCasosForStrip') !== -1, 'orden cognitivo');
